...
export class UpdateMaritalStatusHandler {
  constructor(
  ...
  async execute(command: UpdateMaritalStatusCommand): Promise<void> {
    const maritalStatus = MaritalStatus.create({ ...command.marital_status_dto });
    const maritalStatusId = maritalStatus.getId();
    if (!maritalStatusId) {
      throw new Error(`MaritalStatus id is undefined`);
    }
    const foundMaritalStatus = await this.readRepository.getOneById(maritalStatusId);
    if (!foundMaritalStatus) {
      throw new NotFoundException('MaritalStatus', maritalStatusId.value().toString());
    }
