...
export class DeleteMaritalStatusHandler {
  constructor(
  ...
  async execute(command: DeleteMaritalStatusCommand): Promise<void> {
    const maritalStatus = await this.readRepository.getOneById(
      new MaritalStatusId(command.id),
    );
    if (!maritalStatus) {
    ...
    await this.repository.delete(maritalStatusId);
  }
}
