...
export class Register<T extends FileUpload> {
    ...
    async run(register_dto: RegisterDto<T>, provider_storage_code: string): Promise<void> {
        // 1. Cargar storage file
        const storageFileDto = new StorageFilesContentFile<T>(register_dto.file_img);
