...
export class StorageUploadService<T extends FileUpload> {
    ...
    async run(storage_file_content: T, provider_storage_code: string) : Promise<StorageFiles<T>> {
        return await this.storageUpload.run(storage_file_content, provider_storage_code);
    }
}